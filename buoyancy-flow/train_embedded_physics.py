...
# config_jax.update('jax_disable_jit', True)
...
def gradient_forward_fn_manual(probability_flow_correction_fn, simulation_metadata, update=1, rollout_noise=0.0):
    ...
    def model_loss_grad_1(params, t_sim_train, state, target, obstacles, rng):
        ...
        for n in range(target_smoke.shape[0]-1):
            ...
            correction_smoke, correction_vel_x, correction_vel_y = probability_flow_correction_fn(params, [state_smoke, state_vel_x, state_vel_y, state_mask], jnp.array(t_sim_batch)) 
            
            state_smoke, state_vel_x, state_vel_y = physics_value_fn([state_smoke, state_vel_x, state_vel_y], obstacles, t_sim_batch) 
            ...
            if rollout_noise > 0:
                ...
                state_smoke = state_smoke + rollout_noise * jnp.sqrt(DT) * jax.random.normal(rng, shape=state_smoke.shape, dtype=jnp.float32)
                rng, _ = jax.random.split(rng)
                state_vel_x = state_vel_x + rollout_noise * jnp.sqrt(DT) * jax.random.normal(rng, shape=state_vel_x.shape, dtype=jnp.float32)
                rng, _ = jax.random.split(rng)
                state_vel_y = state_vel_y + rollout_noise * jnp.sqrt(DT) * jax.random.normal(rng, shape=state_vel_y.shape, dtype=jnp.float32)
                rng, _ = jax.random.split(rng)
    ...
    def model_loss_grad_2(params, t_sim_train, state, target, obstacles, rng):
        ...
        for n in range(target_smoke.shape[0]-1):
            ...
            state_smoke_forward, state_vel_x_forward, state_vel_y_forward = physics_value_fn([state_smoke, state_vel_x, state_vel_y], obstacles, t_sim_batch) 
            ...
            correction_smoke, correction_vel_x, correction_vel_y = probability_flow_correction_fn(params, [state_smoke, state_smoke_delta, state_vel_x, state_vel_x_delta, state_vel_y, state_vel_y_delta, state_mask], jnp.array(t_sim_batch)) 
            
            
            
            state_smoke = state_smoke + DT * correction_smoke + state_smoke_delta
            state_vel_x = state_vel_x + DT * correction_vel_x  + state_vel_x_delta
            state_vel_y = state_vel_y + DT * correction_vel_y + state_vel_y_delta
...
def gradient_backward_fn_manual(probability_flow_correction_fn, simulation_metadata, type=2, update=1, rollout_noise=0.0):
    ...
    def model_loss_grad_1(params, t_sim_train, state, target, obstacles, rng):
        ...
        for n in range(target_smoke.shape[0]-1):
            ...
            state_smoke = state_smoke - state_smoke_delta - DT * correction_smoke
            state_vel_x = state_vel_x - state_vel_x_delta - DT * correction_vel_x
            state_vel_y = state_vel_y - state_vel_y_delta - DT * correction_vel_y
...
def log_videos(log_dict, params_, model_params, inference_dict, generator):
    ...
    log_dict = save_simulation_video_score(log_dict, model_params, inference_dict, video_key, video_elem, DT, NSTEPS, params_['inference']['noise'], f'diffusion_1')

    log_dict = save_simulation_video_score(log_dict, model_params, inference_dict, video_key, video_elem, DT, NSTEPS, params_['inference']['noise'], f'diffusion_1_probability_flow')
                ...
                if forward_pass:
                    ...
                    for i in range(simulation_steps): # tqdm(n_steps_random):
                        ...
                        model_params_, opt_state, batch_loss, _, rng = train_inner_forward(i, CURRENT_ROLLOUT, 
                                                                                     t_sim_train, data, obstacles, grad_update_dict['forward'], model_params, 
                    ...
                    if batch_n % print_every_batch == 0:
                        ...
                        print(f'Epoch {epoch:3d} Batch {batch_n:4d} Forward loss: {jnp.abs(batch_loss / CURRENT_ROLLOUT):.6f}')
                ...
                if backward_pass:
                    ...
                    for i in range(simulation_steps): # tqdm(n_steps_random):
                        ...
                        model_params_, opt_state, batch_loss, _, rng = train_inner_backward(i, CURRENT_ROLLOUT, 
                                                                                     t_sim_train, data, obstacles, grad_update_dict['backward'], model_params, 
                                                                                     opt_state, noise, rng, dummy)
                        ...
                        sim_backward_loss += batch_loss / CURRENT_ROLLOUT
                    ...
                    if batch_n % print_every_batch == 0:
                        ...
                        print(f'Epoch {epoch:3d} Batch {batch_n:4d} Backward loss: {jnp.abs(sim_backward_loss / CURRENT_ROLLOUT):.6f}')

            if epoch % 1 == 0:
                
                print(f'Epoch {epoch:3d} Average forward loss: {jnp.abs(forward_loss_avg/(n_steps*num_batches)):.6f}')
                print(f'Epoch {epoch:3d} Average backward loss: {jnp.abs(backward_loss_avg/(n_steps*num_batches)):.6f}')
...
def train_phase_2(params_, generator, model_params, opt_state, grad_update_dict, inference_dict, rng):
    ...
    for epoch in range(params_['phase2']['epochs']):
        ...
        for batch_n in range(num_batches):
            ...
            n_steps_list = [(0, start_)] + [(start_ + i * ROLLOUT, ROLLOUT) for i in range(int(np.ceil(num_steps/ROLLOUT - 0.5)))]
            ...
            if forward_pass:
                ...
                for segment in n_steps_list: 
                    ...
                    model_params_, opt_state, batch_loss, end_state, rng = train_inner_forward(i, CURRENT_ROLLOUT, 
                                                                                 t_sim_train, data, obstacles, grad_update_dict['forward'], model_params, 
                                                                                 opt_state, noise, rng, init=end_state)
                ...
                if batch_n % print_every_batch == 0:
                    ...
                    print(f'Epoch {epoch + epochs_phase_1:3d} Batch {batch_n:4d} Forward loss: {jnp.abs(batch_loss / CURRENT_ROLLOUT):.10f}')  
            ...
            if backward_pass:
                ...
                for segment in n_steps_list:
                    ...
                    model_params_, opt_state, batch_loss, end_state, rng = train_inner_backward(i, CURRENT_ROLLOUT, 
                                                                                 t_sim_train, data, obstacles, grad_update_dict['backward'], model_params, opt_state, noise, rng, init=end_state, useInit=useInit)
                ...
                if batch_n % print_every_batch == 0:
                    ...
                    print(f'Epoch {epoch + epochs_phase_1:3d} Batch {batch_n:4d} Backward loss: {jnp.abs(batch_loss / CURRENT_ROLLOUT):.10f}')  

        if epoch % 1 == 0:

            print(f'Epoch {epoch + epochs_phase_1:3d} Average forward loss: {jnp.abs(forward_loss_avg/(num_batches*num_segments)):.10f}')  
            print(f'Epoch {epoch + epochs_phase_1:3d} Average backward loss: {jnp.abs(backward_loss_avg/(num_batches*num_segments)):.10f}')  
        ...
        if (epoch % print_every) == 0:
            ...
            print(f'Epoch {epoch + epochs_phase_1:3d} Percentage stable forward: {count_stable_forward / (num_batches*num_segments):.3f}')  
            print(f'Epoch {epoch + epochs_phase_1:3d} Percentage stable backward: {count_stable_backward / (num_batches*num_segments):.3f}')  
...
def train_inner_forward(i, CURRENT_ROLLOUT, t_sim_train, data, obstacles, grad_update, model_params, opt_state, noise, rng, init, useInit=0.0):
    ...
    model_params, opt_state, batch_loss, end_state = grad_update(model_params, opt_state, [t_sim_train, state, target, obstacles, rng], rng=rng)
    
    return model_params, opt_state, batch_loss, end_state, rng
    
# @partial(jax.jit, static_argnames=['grad_update'])
def train_inner_backward(i, CURRENT_ROLLOUT, t_sim_train, data, obstacles, grad_update, model_params, opt_state, noise, rng, init, useInit=0.0):
    ...
    for DT in [0.01, 0.02, 0.005]:
        ...
        for key in keys:
            ...
            for i in range(3):
                ...
                smoke_state_init = smoke_state[-1][None] + DT * noise * jr.normal(rng, shape=smoke_state[-1][None].shape, dtype=jnp.float32)
                rng, _ = jax.random.split(rng)
                vel_x_state_init = vel_x_state[-1][None] + DT * noise * jr.normal(rng, shape=vel_x_state[-1][None].shape, dtype=jnp.float32)
                rng, _ = jax.random.split(rng)
                vel_y_state_init = vel_y_state[-1][None] + DT * noise * jr.normal(rng, shape=vel_y_state[-1][None].shape, dtype=jnp.float32)
                rng, _ = jax.random.split(rng)
                ...
                # states_backward_rand_2 = eval_backward_score_2([smoke_state_init, vel_x_state_init, vel_y_state_init, mask_state_init], obstacles, simulation_metadata, correction_probability_flow_fn, rng, noise, type=type_, update=update, physics_forward_fn=physics_forward_fn, physics_backward_fn=physics_backward_fn)
                ...
                states_backward_no_noise = eval_backward_score([smoke_state_init, vel_x_state_init, vel_y_state_init, mask_state_init], obstacles, simulation_metadata, correction_reverse_sde_fn, rng, 0.0, type=type_, update=update, physics_forward_fn=physics_forward_fn, physics_backward_fn=physics_backward_fn)

                rng, _ = jax.random.split(rng)
                
                # states_backward_rand_pc_2 = eval_backward_score_2([smoke_state_init, vel_x_state_init, vel_y_state_init, mask_state_init], obstacles, simulation_metadata, correction_probability_flow_fn, rng, noise, type=type_, update=update, physics_forward_fn=physics_forward_fn, physics_backward_fn=physics_backward_fn, corrector_steps=40)
                ...
                data_dict[DT][key][f'reverse_sde_{i}'] = states_backward_rand
                data_dict[DT][key][f'reverse_sde_{i}_pc'] = states_backward_rand_pc
                data_dict[DT][key][f'reverse_sde_no_noise'] = states_backward_no_noise

            data_dict[DT][key]['ground_truth'] = ground_truth[::-1]

            data_dict[DT][key]['no_correction'] = eval_backward_score([smoke_state_init, vel_x_state_init, vel_y_state_init, mask_state_init], obstacles, simulation_metadata, 
            ...
            data_dict[DT][key]['probability_flow_noise'] = eval_backward_score([smoke_state_init, vel_x_state_init, vel_y_state_init, mask_state_init], obstacles, simulation_metadata, 
                                                               correction_probability_flow_fn, rng, noise, correction_coefficient=1.0, type=type_, update=update,
                                                               physics_forward_fn=physics_forward_fn, physics_backward_fn=physics_backward_fn)
            data_dict[DT][key]['probability_flow'] = eval_backward_score([smoke_state_init, vel_x_state_init, vel_y_state_init, mask_state_init], obstacles, simulation_metadata, 
...
def train(
    ...
    training_correction_probability_flow_fn = get_correction_term_probability_flow_score(forward_fn, params_['training']['noise'])
    ...
    if bool(params_['forward_sim']):
        ...
        inference_correction_probability_flow_fn = jax.jit(get_correction_term_probability_flow_score(forward_fn, params_['inference']['noise']))
        inference_correction_reverse_sde_fn = jax.jit(get_correction_term_reverse_sde_score(forward_fn, params_['inference']['noise']))
    else:
        # if no forward sim, then network learns score instead of 1/2 score
        inference_correction_probability_flow_fn = jax.jit(get_correction_term_probability_flow_score(forward_fn, params_['inference']['noise'] / jnp.sqrt(2)))
        inference_correction_reverse_sde_fn = jax.jit(get_correction_term_reverse_sde_score(forward_fn, params_['inference']['noise'] / jnp.sqrt(2)))
    ...
    model_loss_forward_fn = gradient_forward_fn_manual(training_correction_probability_flow_fn, simulation_metadata,
    ...
    model_loss_backward_fn = gradient_backward_fn_manual(training_correction_probability_flow_fn, simulation_metadata,
    ...
    inference_dict = {'probability_flow': inference_correction_probability_flow_fn, 'reverse_sde': inference_correction_reverse_sde_fn, 'physics_forward' : jax.jit(physics_forward_fn), 'physics_backward' : jax.jit(physics_backward_fn), 'physics_backward_type' : params_['physics_backward'], 'update' : params_['update']}
...
def main(params):
    ...
    if params['architecture'] == 'dilated':
        ...
        forward_fn, init_params = get_model(params_['data_shape'], nBlocks=params['architecture_params']['nBlocks'], nFeatures=params['architecture_params']['nFeatures'], update_type=params['update'])
        ...
        forward_fn, init_params = get_model(params_['data_shape'], use_grid=params['architecture_params']['grid'], update_type=params['update'])
...
if __name__== "__main__":
    ...
    parser.add_argument('--start-epoch', default=hyperparameter_defaults['start_epoch'], type=int, help='Epoch to begin training')
    ...
    parser.add_argument('--network-weights', default=hyperparameter_defaults['network_weights'], help='File with weights used for initialization')
    parser.add_argument('--architecture', default=hyperparameter_defaults['architecture'], help='Network architecture')
    parser.add_argument('--training-noise', default=hyperparameter_defaults['training']['noise'], type=float, help='Coefficient for noise during training')
    parser.add_argument('--inference-noise', default=hyperparameter_defaults['inference']['noise'], type=float, help='Coefficient for noise during training')
    parser.add_argument('--inference-is-training', help='Flag if score parameters are the same for inference as for training', action='store_true')
    parser.add_argument('--forward-sim', default=hyperparameter_defaults['forward_sim'], help='Flag whether to learn probability flow ODE during forward simulation', action='store_true')
    parser.add_argument('--rollout-noise', default=hyperparameter_defaults['rollout_noise'], help='Flag whether to include noise in rollouts', type=float)
    parser.add_argument('--test-only', default=hyperparameter_defaults['test_only'], help='Flag whether to only run test', action='store_true')
    ...
    parser.add_argument('--update', default=hyperparameter_defaults['update'], type=int, help='Update step; either 1 for 1-step or 2 for 2-step')
    parser.add_argument('--physics-backward', default=hyperparameter_defaults['physics_backward'], type=int, help='What backward physics to use; either 1 for reusing negative forward physics or 2 for time integration')
