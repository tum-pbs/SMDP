...
def predict(generator, params, forward_fn, model_weights , data_keys, rng, realizations = 8):
        ...
        for elem in forward_:
            elem = elem + params['fno']['noise'] * jr.normal(rng, shape=elem.shape)
        ...
        pred_fn = lambda weights, data, rng: forward_fn(weights, rng, data)
        pred_fn = jax.jit(pred_fn)
        
        for i in range(realizations):
          
            print(jnp.expand_dims(forward_noise, axis=1).shape)
